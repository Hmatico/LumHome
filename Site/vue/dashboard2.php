<html lang="fr">
  <head>
	<meta http-equiv="content-type" content="text/html; charset=UTF-8">
	<meta name="viewport" content="width=device-width, maximum-scale=1"/>
	<title>Dashboard</title>
	<link rel="stylesheet" href="dashboard2.css">
	<link rel="stylesheet" href="ui.colorpicker.css"/>
	<script language="JavaScript" src="jQuery.js"></script>
	<script language="JavaScript" src="jQuery.colorpicker.js"></script>
  </head>
  <body>
<!DOCTYPE html>
<html>
    <head>
        <meta charset="utf-8" />
        <title>LumHome</title>
    </head>
    <body>
	<?php include "entete.html";?>
	<div class = "dhabitatprincipal">
		<div class="dleftcontainer">
			<div class="dhabitatactuel">
				<form method="post" action="">
					<select class = "dselecthabitat">
					   <option value="france">Habitation 1(principal)</option>
					   <option value="espagne">Habitation 2</option>
				   </select>
				</form>
			</div>
			<div class="dtteteindre">
				<label class="switch"><input type="checkbox"><span class="slider round"></span></label>
				<a class="dtxteteindre"> Tout éteindre</a>
			</div>
		</div>
		<div class="drightcontainer">
			<div class="dligne">
				<ul>
					<li><div class="detatpiece">Chambre de Didier : <a class = "detat"> allo </a></div></li>
					<li><div class="detatpiece">piece</div></li>
					<li><div class="detatpiece">piece: <a class = "detat"> allumé </a></div></li>
					<li><div class="detatpiece">piece</div></li>
					<li><div class="detatpiece">piece</div></li>
					<li><div class="detatpiece">piece</div></li>
				</ul>
			</div>
			<div class="dligne">
				<ul>
					<li><div class="detatpiece">piece(Noël)</div></li>
					<li><div class="detatpiece">piece</div></li>
					<li><div class="detatpiece">piece</div></li>
					<li><div class="detatpiece">piece</div></li>
					<li><div class="detatpiece">piedgophlhhce: <a class = "detat"> Noël </a></div></li>
					<li><div class="detatpiece">piece</div></li>
				</ul>
			</div>
			<div class="dligne">
				<ul>
					<li><div class="detatpiece">piece</div></li>
					<li><div class="detatpiece">piece</div></li>
					<li><div class="detatpiece">piece</div></li>
					<li><div class="detatpiece">piece</div></li>
					<li><div class="detatpiece">piece</div></li>
					<li><div class="detatpiece">piece</div></li>
				</ul>
			</div>
		</div>
	</div>
</html>